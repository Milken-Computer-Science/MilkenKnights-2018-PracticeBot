...
public class SpectrumJeVois {
	 ...
	 * 0 = OK in response -1 = ERR in response -2 = No token found before timeout_s
	 */
	...
	public void startCameraStream1() {
		...
		try {
			...
			visionCam.setVideoMode(PixelFormat.kMJPEG, MAPPING_WIDTH_PXL_1, MAPPING_HEIGHT_PXL_1,
					MAPPING_FRMRT_FPS_1);
	...
	public void startCameraStream2() {
		...
		try {
			...
			visionCam = new UsbCamera("VisionProcCam", JEVOIS_CAM_NUMBER);
			visionCam.setVideoMode(PixelFormat.kMJPEG, MAPPING_WIDTH_PXL_2, MAPPING_HEIGHT_PXL_2,
					MAPPING_FRMRT_FPS_2);
			camServer = new MjpegServer("VisionCamServer", MJPG_STREAM_PORT);
			camServer.setSource(visionCam);
			camStreamRunning = true;
	...
	/**
	 * This is the main perodic update function for the Listener. It is intended to be run in a
	 * background task, as it will block until it gets packets.
	 */
	...
	/**
	 * Mostly for debugging. Blocks execution while serOutEnable and just prints all serial characters
	 * to the console. It might print a different message too if nothing comes in.
	 */
	...
	/**
	 * Blocks thread execution till we get a valid packet from the serial line or timeout. Return
	 * values: String = the packet null = No full packet found before timeout_s
	 */
	...
	/**
	 * Read bytes from the serial port in a non-blocking fashion Will return the whole thing once the
	 * first "OK" or "ERR" is seen in the stream. Returns null if no string read back yet.
	 */
